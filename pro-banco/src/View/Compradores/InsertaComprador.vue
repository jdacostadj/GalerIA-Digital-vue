<template>
    ...
    <fieldset>
        ...
        <label for="">Crear comprador</label>

        <form @submit.prevent="insertaComprador">
            <div>
...
const mensaje = ref("")
const insertaComprador = async ()=> {
    
    try{
        const respuesta= await fetch("http://localhost:8080/compradores",
        {
        method:"POST",
            headers:{
        ...
        if(!respuesta.ok){
            throw new Error("No se ha podido crear comprador (InsertaComprador)")
        }

        Swal.fire({
            title: "Registro ok",
            text: "Ha creado comprador de forma correcta",
            icon: "success",
